...
class CodeGrafter():
    ...
    def _lookup_bytes(self, table, fname):
        try:
            ...
            if str(e)[1:-1] == fname:
                raise CodeGraftingUnsupportedFunc(f'Function {fname} not supported')
            else:
                raise CodeGraftingUnsupportedArch(f'{self.bits}-bit {self.cpu} not supported')
    ...
    def _addSegments(self, mem=0x4000000):
        ...
        def add_stub_func(va, sc, nm):
            ...
            cmt = f'{nm} implementation generated by FLARE Code Grafter'
    ...
    def _get_imp_for_register_call(self, va_call, nm=None):
        ...
        if va_mov := mykutils.find_instr(
            va_call, 'up', 'mov', [(0, 1, reg), (1, 2, None)]
        ):
            return (
                None
                if nm and (nm not in idc.print_operand(va_mov, 1))
                else idc.get_operand_value(va_mov, 1)
            )
        ...
        def do_patch_call(va):
            ...
            if idc.get_operand_type(va, 0) == ida_ua.o_mem:
                ...
                if va_imp:
                    ...
                    logger.warn(f'Could not find import to patch call at {phex(va)}')
                ...
                if abs(call_offset_loc - stub_loc) > 0x100000000:
                    msg = f'Call site at {phex(va)} too far from {self._stubname(nm)} ({phex(stub_loc)})'
    ...
    def _unpatchCalls(self, grafted_segs):
        def do_unpatch_call(va_callsite):
            ...
            if newcmt.startswith(g_patched_call_cmt):
                ...
                newcmt = '' if newcmt.find('\n') == -1 else newcmt[newcmt.find('\n') + 1:]
...
def patch_import(va, target):
    ...
    if is_call:
        ...
        logger.warn(f'Not a call instruction at {phex(va)}')
        return False
...
def patch_call(va, new_nm):
    ...
    if not ok:
        logger.warn(f'Failed assembling {phex(va)}: {new_asm}')
        return False
    ...
    if orig_opcode_len < new_code_len:
        logger.warn(
            f'Not enough room or wrong opcode type to patch {phex(va)}: {new_asm}'
        )
